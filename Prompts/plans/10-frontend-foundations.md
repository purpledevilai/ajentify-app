...
9. **Cross-store dependency by direct module import.** `AgentBuilderStore` imports `agentsStore` (`store/AgentBuilderStore.ts:11`). Today fine, tomorrow a circular import.
10. **`next.config.ts` ships a `child_process: false` webpack fallback.** Symptom of a Node module being pulled into the client bundle (almost certainly transitive through `aws-amplify`). Should be unnecessary on the public side after the layout split.
...
- [ ] `AuthStore.signOut()` is a thin wrapper: it calls Amplify's `awsSignOut()`, deletes the `aj_signed_in` cookie (`document.cookie = 'aj_signed_in=; Path=/; Max-Age=0'`), then calls the `resetAll` callback it received in its constructor, then sets `signedIn = false`. **Do not** restore the manual list of `xxxStore.reset()` calls; that's exactly the pattern this deliverable removes. (Render-phase `redirect('/signin')` from `<DashboardBoot>` re-rendering on the `signedIn` flip handles the navigation; deliverable F.1.)
...
- [ ] Replace every `router.push(staticPath)` with `<Link href={staticPath}>`. Reserve `router.push` for navigations triggered by post-action redirects (e.g. "after `createAgent()`, route to its detail page").
- [ ] Replace the manual `router.prefetch` calls (`agents/page.tsx:337`, `tools/page.tsx:285`, `sres/page.tsx:347`, `documents/page.tsx:273`, `contexts/page.tsx:82`, plus any others surfaced by the sweep) with `<Link prefetch>` (the default). The boot-time prefetch in I.2 covers the top-level routes; per-link prefetch covers the rest.
...
- [ ] Investigate the `child_process: false` webpack fallback in `next.config.ts`. Bundle-analyze to find the offending import; it's almost certainly transitive through `aws-amplify`. After deliverable B moves Amplify out of the public bundle, the public side no longer needs the fallback.
- [ ] If the offending import is only on the authenticated side, leave the fallback but add a comment naming the package and the chain (`aws-amplify` → `<X>` → `child_process`). If it's gone entirely (because the layout split shed it), delete the workaround.
...
- [ ] No builder store exceeds 300 lines (Tool, Agent, SRE, JsonDocument). `ChatPageBuilderStore` is exempt — it's deprecated and untouched.
