...
import { exchangeGoogleCalendarCode } from '@/api/integration/exchangeGoogleCalendarCode';
import {
  Box,
  Flex,
  Heading,
...
function GoogleCalendarAuthCallbackContent() {
  ...
  async function exchangeCode(code: string, orgId: string | null) {
    try {
      await exchangeGoogleCalendarCode(code, orgId || undefined);
      setStatus('success');
...
export default function GoogleCalendarAuthCallbackPage() {
  return (
    <Suspense
      fallback={
        <Flex
    ...
    >
      <GoogleCalendarAuthCallbackContent />
    </Suspense>
  );
}
